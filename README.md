...
- `RDL_STRICT_FONTS=false` permits legacy PDF development substitutions only. It never relaxes
  `DOCX_EDITABLE`, which must embed every consumed face.
- `DOCX_EDITABLE` embeds every consumed font face after checking `OS/2.fsType`. A missing face returns
  `FONT_MISSING`; a face that forbids editable embedding returns `FONT_EMBEDDING_FORBIDDEN`.

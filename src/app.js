...
export async function buildApp(options = {}) {
  ...
  app.post('/v1/analyze', async (request, reply) => {
    ...
    const { rdlBuffer } = await readInput(request, config);
    const result = analyzeRdl(rdlBuffer, { maxRdlBytes: config.maxRdlBytes, maxXmlNodes: config.maxXmlNodes, maxXmlDepth: config.maxXmlDepth });
